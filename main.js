...
ipcMain.handle('bridge:start',async(_,opts)=>{
  try{
    ...
    bridge.onDJMStatus=f=>_send('bridge:djm',{faders:f.channel||f, onAir:f.onAir, eq:f.eq, xfader:f.xfader, masterLvl:f.masterLvl, boothLvl:f.boothLvl, hpLevel:f.hpLevel, hpCueCh:f.hpCueCh, chExtra:f.chExtra, pktType:f.pktType, pktLen:f.pktLen, rawHex:f.rawHex});

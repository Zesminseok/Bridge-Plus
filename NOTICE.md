...
BRIDGE+ is an independent third-party application created to enable interoperability between DJ hardware and visual / lighting software. It is **not affiliated with, endorsed by, or sponsored by AlphaTheta Corporation, Pioneer DJ, Tom Cosm Technologies, or any other mentioned party.**
...
BRIDGE+ is released under the Apache License 2.0 (see `package.json` or repository root for details).
...
BRIDGE+는 DJ 하드웨어와 비주얼/조명 소프트웨어 간의 상호운용성을 지원하기 위해 제작된 독립적인 서드파티 애플리케이션입니다.  
본 소프트웨어는 **AlphaTheta Corporation, Pioneer DJ, Tom Cosm Technologies 또는 기타 언급된 어떠한 기업과도 제휴, 승인 또는 후원을 받지 않습니다.**
...
BRIDGE+는 Apache License 2.0 으로 배포됩니다. (`package.json` 또는 저장소 루트 참조)

# Graph Report - /Users/zes2021/Library/CloudStorage/Dropbox/claude_projects/bridge-clone  (2026-04-26)

## Corpus Check
- 29 files · ~225,054 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 801 nodes · 1187 edges · 51 communities detected
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 115 edges (avg confidence: 0.79)
...
6. `getAllInterfaces()` - 13 edges
7. `buildHdr()` - 13 edges
8. `Card AFTER — 리파인 (Player 1 PLAY, hero art 확대, title 위로 swap, BPM meta 9px, TC 조정, safety u3 12px)` - 12 edges
9. `AFTER (이후) — 개선된 Row 레이아웃 스크린샷` - 11 edges
10. `AFTER card (리파인된)` - 11 edges

## Surprising Connections (you probably didn't know these)
- `push()` --calls--> `parsePcapng()`  [INFERRED]
  /Users/zes2021/Library/CloudStorage/Dropbox/claude_projects/bridge-clone/main.js → tools/pcap-replay.js
...
Cohesion: 0.08
Nodes (13): BridgeCore, dbArg4(), dbBinary(), dbBuildArtResponse(), dbBuildMenuItem(), dbBuildMsg(), dbNum1(), dbNum2() (+5 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (22): gcPlayers(), getActualPitch(), getFaderPitch(), getVuPeak(), getVuPeakNorm(), handleBeatPacket(), handleDJMStatusPacket(), handleKeepalivePacket() (+14 more)
...
Cohesion: 0.08
Nodes (19): detectBroadcastFor(), getAllInterfaces(), _getHWPortMap(), pdjlBroadcastTargets(), _resolveDbgLogPath(), buildHdr(), mkAppResp(), mkDataMeta() (+11 more)
...
Cohesion: 0.08
Nodes (11): ArtnetEngine, createWindow(), doQuit(), getLocalIp(), interfaceSignature(), loadBounds(), saveBounds(), sendArtTimeCode() (+3 more)
...
Cohesion: 0.09
Nodes (19): _dbgLog(), nxs2BeatCountToMs(), shouldKeepPredictedBeatAnchor(), test(), buildBridgeNotifyPacket(), buildDbServerKeepalivePacket(), buildDjmSubscribePacket(), buildPdjlBridgeClaimPacket() (+11 more)
...
Cohesion: 0.35
Nodes (11): _findByNameUnix(), _findByNameWin(), _findByPortUnix(), _findByPortWin(), isProtected(), _killUnix(), _killWin(), _pidNameUnix() (+3 more)
...
Cohesion: 0.4
Nodes (9): _mxBuildBody(), _mxDetectProfile(), _mxEnsureAuxKnobs(), _mxKnobSVG(), _mxKnobUpdate(), _mxMountKnob(), _mxSetAuxPill(), _mxSetAuxToggle() (+1 more)
...
- **Thin community `Community 34`** (2 nodes): `waveform-analysis.js`, `buildRGBWaveform()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 35`** (2 nodes): `jsString()`, `electron_analyze_waveform.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 36`** (2 nodes): `test()`, `license-service.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (2 nodes): `test()`, `metadata-bpm.test.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (2 nodes): `registerLicenseIpc()`, `ipc-license.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (2 nodes): `registerArtnetIpc()`, `ipc-artnet.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (2 nodes): `Proposal C - Minimal Tactical (light theme)`, `3-column body layout (260/flex/320)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (2 nodes): `Information priority: Timecode > Waveform > Track ID > BPM > others`, `Proposal G - Timecode First (Monolith) information hierarchy`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (2 nodes): `Preview Develop - G.ROW + Waveform 2-band + Beat Ring + Knob Arc`, `Preview v2 - TOWER/CARD/ROW developed with channel strip + fader`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (1 nodes): `preload.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (1 nodes): `rename-stub.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (1 nodes): `v0.5.0 Initial release`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (1 nodes): `graphify workflow rules`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (1 nodes): `PDJL key facts (ports, broadcast, header, BPM encoding)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (1 nodes): `Default Album Artwork Placeholder (Vinyl Disc)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (1 nodes): `Default Album Art (Vinyl/CD Placeholder)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (1 nodes): `Default Album Art (Vinyl Record Placeholder)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `push()` connect `Community 0` to `Community 3`, `Community 5`, `Community 7`, `Community 8`, `Community 23`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `BridgeCore` connect `Community 0` to `Community 8`, `Community 3`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `ArtnetEngine` connect `Community 5` to `Community 7`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Are the 34 inferred relationships involving `push()` (e.g. with `_resolveDbgLogPath()` and `_dbgLog()`) actually correct?**
  _`push()` has 34 INFERRED edges - model-reasoned connections that need verification._
